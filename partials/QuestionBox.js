  ...
  return (
    <div className="">
      <div className="Question_info">
        ...
        Asked By {props.creator}  At {props.Time}
      </div>
      <div className="px-5 py-3">
        ...
        <div className="rounded-3">
          <div className="d-flex">
            ...
            <div className='mb-4'> 
            {props.tags.map((tag , key)=>(<span className="px-2 rounded-3 bg-dark mx-1 text-light fw-light" key={key}>{tag}</span>))}
            </div>
        </div>
        <button className="btn btn-light border mx-1" onClick={editQuestion}>edit</button>
      <button className="btn btn-light border mx-1" onClick={()=>supQuestion(props.id)}>Delete</button>
      </div>
      <hr></hr>
    </div>
  );
